...
- **`thinking_config` is the nested exception:** a mapping or SDK `ThinkingConfig` merges only the fields explicitly supplied into the provider's computed typed config. Empty values leave computed fields alone; this preserves independent fields such as `include_thoughts`. Both SDK spellings work (`thinking_level`/`thinkingLevel`, `thinking_budget`/`thinkingBudget`, and `include_thoughts`/`includeThoughts`).
