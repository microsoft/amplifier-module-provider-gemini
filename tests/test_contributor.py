...
Verifies that mount() registers:
  ...
  - a lazy contributor callback on session.cost channel under name 'provider-gemini'
"""
...
async def test_contributor_registered_at_mount(mock_coordinator):
    """mount() must register a contributor on ('session.cost', 'provider-gemini')."""
...
async def test_contributor_accumulates_after_llm_response(mock_coordinator):
    ...
    await accumulate("llm:response", {"provider": "gemini", "usage": {"cost_usd": "0.05"}})
    await accumulate("llm:response", {"provider": "gemini", "usage": {"cost_usd": "0.03"}})
...
async def test_contributor_ignores_none_cost(mock_coordinator):
    ...
    await accumulate("llm:response", {"provider": "gemini", "usage": {"cost_usd": None}})
